...
test.describe('A-5 schema conformance', () => {
  ...
  test('A-5 stylist listing conforms to spec @critical', async ({ api }) => {
  ...
  test('A-5 a missing stylist returns the spec error shape @important', async ({ api }) => {
    const { error, response } = await api.GET('/stylists/{stylistId}', {
      params: { path: { stylistId: 'does-not-exist' } },
    });
    expect(response.status).toBe(404);
    expect(error).toMatchSpec({ path: '/stylists/{stylistId}', method: 'get', status: 404 });
  });
});
